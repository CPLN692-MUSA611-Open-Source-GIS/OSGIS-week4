...
var query5 = _.filter(bakedGoods, function(item){return item["price"] > 4});
...
var query7 = _.groupBy(bakedGoods, function(item){return item["type"]});
...
function printMenuGroups(query, template){
  type = Object.keys(query);
  _.map(type, function(type){
    console.log(type);
    _.map(query[type], function(item){
      console.log(template(item));
    })
  })
}

var template = _.template("<%= name %> ... $<%= price %>");
